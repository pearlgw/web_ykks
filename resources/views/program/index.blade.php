<x-app-layout>
        ...
        <div class="flex flex-1 overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-10">
                ...
                <div class="flex items-center justify-between mb-8">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800">Program</h1>
                        <p class="text-gray-500 mt-1">Manage all program</p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    @forelse($programs as $index => $program)
                                ...
                                <table class="w-full">
                                    <thead>
                                        <tr class="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                                            ...
                                            <th class="px-6 py-4 text-left text-sm font-bold text-gray-700 w-20">
                                                Icon
                                            </th>
                        ...
                        <tr class="hover:bg-indigo-50 transition duration-150 group">
                            ...
                            <td class="px-6 py-4">
                                @if ($program->icon_image)
                                    <img src="{{ asset('storage/' . $program->icon_image) }}"
                                        alt="{{ $program->title }}"
                                        class="w-12 h-12 object-cover rounded-lg border-2 border-gray-200 shadow-sm group-hover:border-blue-400 transition-all duration-200">
                                @else
                                    <div
                                        class="w-12 h-12 bg-gradient-to-br from-gray-200 to-gray-300 rounded-lg flex items-center justify-center">
                            ...
                            <!-- Title -->
                            <td class="px-6 py-4">
                                <span class="text-sm font-semibold text-gray-900">{{ $program->title }}</span>
                            </td>

                            <!-- Description -->
                            <td class="px-6 py-4">
                                <span
                                    class="text-sm text-gray-600">{{ Str::words($program->description, 12, '...') }}</span>
                            </td>

                            <!-- Created At -->
                            <td class="px-6 py-4">
                                <div class="text-sm">
                                    <p class="font-medium text-gray-900">{{ $program->created_at->format('d M Y') }}</p>
                                    <p class="text-gray-500 text-xs">{{ $program->created_at->format('H:i') }}</p>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <button
                                        onclick="openDeleteModal({{ $program->id }}, '{{ addslashes($program->title) }}')"
                        ...
                        @foreach ($programs->getUrlRange(1, $programs->lastPage()) as $page => $url)
                            ...
                            @elseif (
                                $page == 1 ||
                                    ...
                                    ($page >= $programs->currentPage() - 1 && $page <= $programs->currentPage() + 1))
                                <li>
                                    <a href="{{ $url }}"
                                        class="px-3 py-2 text-gray-700 hover:bg-gray-100 rounded-lg transition duration-200 font-medium">{{ $page }}</a>
                                </li>
                            @elseif ($page == $programs->currentPage() - 2 || $page == $programs->currentPage() + 2)
                                <li>
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full transform transition-all animate-scale-in">
            ...
            <div class="bg-gradient-to-r from-red-500 to-rose-600 px-6 py-5 rounded-t-2xl">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-xl font-bold text-white">Delete Program</h3>
            ...
            <div class="p-6">
                <div class="mb-6">
                    <p class="text-gray-700 mb-4">Are you sure you want to delete this program?</p>
                    <div class="bg-gradient-to-r from-red-50 to-rose-50 border border-red-200 rounded-lg p-4">
                        <p class="text-xs font-semibold text-red-900 mb-2 uppercase tracking-wide">Program Title</p>
                        <p class="text-sm font-medium text-red-700" id="deleteProgramTitle"></p>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-200 rounded-lg p-4 mb-6">
                    <div class="flex gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-yellow-700 leading-relaxed">This will permanently delete the program
    ...
    <script>
        ...
        // Modal Functions
        function openDeleteModal(programId, programTitle) {
            const modal = document.getElementById('deleteModal');
            const form = document.getElementById('deleteForm');
            const nameElement = document.getElementById('deleteProgramTitle');

            form.action = `/program/${programId}`;
            nameElement.textContent = programTitle;
            ...
            if (searchInput) {
                searchInput.addEventListener("input", function() {
                    ...
                    timer = setTimeout(() => {
                        ...
                        fetch(`/program?search=${encodeURIComponent(query)}`, {
                                headers: {
